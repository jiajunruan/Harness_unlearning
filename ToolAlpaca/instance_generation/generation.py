...
parser.add_argument("--instruction_length", type=int, default=-1)
...
if args.llm is None or args.llm.lower() in ["gpt3", "gpt-3"]:
    ...
    # ToolAlpaca checkpoints use a SentencePiece tokenizer.  With the pinned
    # 2023 Transformers stack, fast-tokenizer conversion can be extremely slow
    # or fail against newer protobuf runtimes, so keep the native slow tokenizer.
...
for api_idx, api in tqdm(enumerate(api_data)):
    ...
    if input_valid and output_valid:
        ...
        if args.instruction_length != -1:
            instruction_end = args.instruction_offset + args.instruction_length
        instructions = api["Instructions"][args.instruction_offset:instruction_end]
        for idx, inst in enumerate(instructions, start=args.instruction_offset):
