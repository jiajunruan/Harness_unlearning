...
def _api_params(params, model):
    # gpt-5.x chat models (e.g. gpt-5.6-luna) reject max_tokens (must use
    # max_completion_tokens) and only accept the default temperature (1).
    if model.startswith("gpt-5"):
        params = dict(params)
        params["temperature"] = 1
        mct = params.pop("max_tokens", None)
        if mct is not None:
            params["max_completion_tokens"] = mct
    # Drop explicit nulls: the API rejects e.g. {"max_tokens": null}.
...
def openai_chat_completions(
    ...
    **kwargs
    ...
    partial_func = partial(_completion_with_retry, **params)
